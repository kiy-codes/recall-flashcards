...
window.RecallLibrary = {
  ...
  deck(id) { return state.sets.find(set => set.id === id) || null; },
  activeDeck() { return activeSet(); },
  addSharedDeck(snapshot) {
    const copy = RecallShareCore.copyDeck(snapshot, makeId);
...
function setAppView(view = 'home') {
  ...
  focusHomeNavigation?.(view === 'cards' || view === 'deck' ? 'library' : view);
...
function load() {
  try {
    ...
    if (saved?.sets?.length) {
      ...
      state.folders = Array.isArray(saved.folders) ? saved.folders.map((folder, index) => ({ ...folder, order: Number.isFinite(folder.order) ? folder.order : index, color: safeFolderColor(folder.color, FOLDER_COLORS[index % FOLDER_COLORS.length]) })) : [];
  ...
  } catch { /* Keep the original local backup for recovery; never erase it on a parse error. */ }
...
function renderDeck() {
  ...
  elements.bulkMoveDeck.innerHTML = state.sets.filter(set => set.id !== deck?.id).map(set => `<option value="${escapeHtml(set.id)}">${escapeHtml(set.name)}</option>`).join('');
  elements.shareDeckOptions.innerHTML = state.sets.map(set => `<label><input type="checkbox" value="${escapeHtml(set.id)}" ${set.id === deck?.id ? 'checked' : ''}/><span>${escapeHtml(set.name)}</span></label>`).join('');
  ...
  visibleDeckCards.forEach(card => {
    ...
    const select = state.selectingCards ? `<label class="card-check"><input type="checkbox" data-select-card="${escapeHtml(card.id)}" ${state.selectedCardIds.has(card.id) ? 'checked' : ''} aria-label="Select card" /></label>` : '';
  ...
  elements.bulkMoveDeck.innerHTML = state.sets.filter(set => set.id !== activeSet()?.id).map(set => `<option value="${escapeHtml(set.id)}">${escapeHtml(set.name)}</option>`).join('');
  elements.shareDeckOptions.innerHTML = state.sets.map(set => `<label><input type="checkbox" value="${escapeHtml(set.id)}" ${set.id === activeSet()?.id ? 'checked' : ''}/><span>${escapeHtml(set.name)}</span></label>`).join('');
...
function testTimerMarkup(test) { const remaining = RecallTest.remainingMs(test); const hasLimit = Number(test.config.timeLimitSeconds) > 0; const urgent = hasLimit && remaining <= 60000; return test.config.timed ? `<span class="test-timer ${urgent ? 'urgent' : ''}" id="testElapsed" data-test-deadline="${escapeHtml(test.deadlineAt || '')}">${hasLimit ? testFormatDuration(remaining) : testFormatDuration(Date.now() - new Date(test.startedAt))}</span>` : `<span class="test-timer">${testFormatDuration(Date.now() - new Date(test.startedAt))} elapsed</span>`; }
...
function folderControls(folder, manual) {
  if (!folder) return '';
  return `<div class="folder-actions"><input class="folder-colour" data-folder-color="${escapeHtml(folder.id)}" type="color" value="${safeFolderColor(folder.color)}" title="Change folder colour"/><button class="library-icon-button" data-rename-folder="${escapeHtml(folder.id)}" type="button" title="Rename folder">✎</button>${manual ? `<button class="library-icon-button" data-move-folder="${escapeHtml(folder.id)}" data-direction="up" type="button" title="Move folder up">↑</button><button class="library-icon-button" data-move-folder="${escapeHtml(folder.id)}" data-direction="down" type="button" title="Move folder down">↓</button>` : ''}<button class="library-icon-button library-delete-button" data-delete-folder="${escapeHtml(folder.id)}" type="button" title="Delete folder">×</button></div>`;
}

function setActions(set, manual) { const id = escapeHtml(set.id); return `<div class="library-item-actions"><button class="library-icon-button" data-share-set="${id}" type="button" title="Share deck" aria-label="Share ${escapeHtml(set.name)}">↗</button><button class="library-icon-button" data-rename-set="${id}" type="button" title="Rename set">✎</button>${manual ? `<button class="library-icon-button" data-move-set="${id}" data-direction="up" type="button" title="Move set up">↑</button><button class="library-icon-button" data-move-set="${id}" data-direction="down" type="button" title="Move set down">↓</button>` : ''}<button class="library-icon-button library-delete-button" data-delete-set="${id}" type="button" title="Delete set">×</button></div>`; }
...
function openSharePreview(payload) {
  if (!payload || !/^recall-share-v[12]$/.test(payload.format || '') || !Array.isArray(payload.sets) || payload.sets.length > 100 || !Array.isArray(payload.folders || []) || (payload.folders || []).length > 100) throw new Error('That share file is invalid or too large.');
  payload = { sets: payload.sets.map(set => ({ ...RecallShareCore.validateSnapshot(set), folderId: typeof set.folderId === 'string' ? set.folderId.slice(0, 128) : null })), folders: (payload.folders || []).filter(folder => folder && typeof folder.name === 'string').map(folder => ({ id: String(folder.id || '').slice(0, 128), name: folder.name.slice(0, 70), color: safeFolderColor(folder.color) })) };
...
function csvEscape(value, delimiter) { let text = String(value ?? ''); if (/^[\s\u0000-\u001f]*[=+@-]/.test(text)) text = `'${text}`; return text.includes('"') || text.includes(delimiter) || /[\r\n]/.test(text) ? `"${text.replace(/"/g, '""')}"` : text; }
...
function exportShare() { const ids = [...elements.shareDeckOptions.querySelectorAll('input:checked')].map(input => input.value); const sets = state.sets.filter(set => ids.includes(set.id)); if (!sets.length) return showToast('Choose at least one deck to share.'); try { const payload = { format: 'recall-share-v2', exportedAt: new Date().toISOString(), folders: [], sets: sets.map(RecallShareCore.snapshotDeck) }; downloadLocalFile('recall-decks.recall', JSON.stringify(payload, null, 2), 'application/json'); showToast(`${sets.length} deck${sets.length === 1 ? '' : 's'} exported without private notes or progress.`); } catch (error) { showToast(error.message); } }
...
function confirmImportFlow() {
  ...
  if (draft.kind === 'share') {
    ...
    save(); buildQueue(); closeImportPreview(); if (state.currentView === 'cards') setAppView('deck'); showToast(`${imported} shared card${imported === 1 ? '' : 's'} imported.`); return;
  }
  ...
  closeImportPreview(); elements.importInput.value = ''; elements.importStatus.textContent = `${imported} card${imported === 1 ? '' : 's'} imported to ${destination.name}.`; if (state.currentView === 'cards') setAppView('deck'); showToast(`${imported} card${imported === 1 ? '' : 's'} imported.`);
}
...
$('#addCardForm').addEventListener('submit', event => { event.preventDefault(); const set = activeSet(); set.tags = cleanTags([...(set.tags || []), ...cleanTags(elements.tagsInput.value)]); const card = { front: elements.frontInput.value, back: elements.backInput.value, hint: elements.hintInput.value, notes: elements.notesInput.value }; const duplicate = findDuplicate(card); if (duplicate && !confirm('A card with the same two sides already exists. Keep another copy?')) return; const added = addCards([card]); if (added) { elements.frontInput.value = ''; elements.backInput.value = ''; elements.tagsInput.value = ''; elements.hintInput.value = ''; elements.notesInput.value = ''; setAppView('deck'); showToast('Flashcard added to your deck.'); } });
$('#openAddCardsBtn').addEventListener('click', () => { setAppView('cards'); setTimeout(() => elements.frontInput.focus(), 0); });
$('#closeAddCardsBtn').addEventListener('click', () => setAppView('deck'));
...
elements.fileImport.addEventListener('change', async () => { const file = elements.fileImport.files[0]; if (!file) return; try { if (file.size > 2097152) throw new Error('Import files must be 2 MiB or smaller.'); const text = await file.text(); if (/\.recall$/i.test(file.name)) { openSharePreview(JSON.parse(text)); } else { const delimiter = /\.tsv$/i.test(file.name) ? '\t' : ','; const rows = parseDelimited(text, delimiter); if (rows.length > 2001) throw new Error('Import files can contain at most 2,000 cards.'); if (rows.length > 1) { state.importDraft = rowsToImportDraft(rows); renderImportPreview(); elements.importPreview.hidden = false; } else showToast('That file has no importable rows.'); } } catch (error) { showToast(['Import files must be 2 MiB or smaller.', 'Import files can contain at most 2,000 cards.', 'That share file is invalid or too large.'].includes(error.message) ? error.message : 'Could not read this import file. Check its format and try again.'); } finally { elements.fileImport.value = ''; } });
...
function handleLibraryClick(event) {
  const shareSetId = event.target.closest('[data-share-set]')?.dataset.shareSet;
  if (shareSetId) { window.dispatchEvent(new CustomEvent('recall:share-deck', { detail: { deckId: shareSetId } })); return; }
  ...
  if (intent === 'cards') { setAppView('deck'); return; }
...
function openDeckChooser(intent = 'library', search = '') {
  ...
  openFullLibrary(); focusHomeNavigation(intent === 'test' ? 'test' : intent === 'study' ? 'study' : 'library');
}
...
function testDeckChoices(selected = [activeSet()?.id]) {
  return state.sets.map(set => `<label class="test-check"><input data-test-deck type="checkbox" value="${escapeHtml(set.id)}" ${selected.includes(set.id) ? 'checked' : ''}/><span>${escapeHtml(set.name)}</span><small>${set.cards.length} cards</small></label>`).join('');
}
function testFolderChoices() {
  return state.folders.map(folder => `<label class="test-check"><input data-test-folder type="checkbox" value="${escapeHtml(folder.id)}"/><span><i style="background:${safeFolderColor(folder.color)}"></i>${escapeHtml(folder.name)}</span><small>${state.sets.filter(set => set.folderId === folder.id).length} decks</small></label>`).join('') || '<p class="test-muted">No folders yet.</p>';
}
...
function renderLibrary() {
  ...
  libraryGroups(filter, mode).forEach(({ folder, sets, ungrouped }) => { const group = document.createElement('section'); group.className = `library-folder ${ungrouped ? 'ungrouped-folder' : ''}`; group.dataset.dropFolder = folder?.id || ''; const color = safeFolderColor(folder?.color); const title = ungrouped ? 'Ungrouped sets' : folder.name; const collapsed = Boolean(folder?.collapsed); group.innerHTML = `<div class="library-folder-head" style="--folder-colour:${color}"><button class="folder-toggle" data-toggle-folder="${escapeHtml(folder?.id || '')}" type="button" ${ungrouped ? 'disabled' : ''}><span class="folder-symbol">●</span><span>${escapeHtml(title)}</span><i>${collapsed ? '›' : '⌄'}</i></button>${folderControls(folder, manual)}</div>`; if (!collapsed) sets.forEach(set => { const row = document.createElement('div'); row.className = 'library-set-row'; row.draggable = true; row.dataset.dragSet = set.id; row.innerHTML = `<button class="set-entry ${set.id === state.activeSetId ? 'active' : ''} ${resolvedDeckSubject(set).colour ? 'has-subject' : ''}" ${deckAccentStyle(set)} data-set="${escapeHtml(set.id)}" type="button"><span>${escapeHtml(set.name)}</span>${deckSubjectDots(set)}<small>${set.cards.length} ${set.cards.length === 1 ? 'card' : 'cards'}</small></button>${setActions(set, manual)}`; group.append(row); }); elements.libraryTree.append(group); });
...
function renderFullLibrary() {
  ...
  libraryGroups(filter, mode).forEach(({ folder, sets, ungrouped }) => { const group = document.createElement('section'); group.className = `full-folder-group ${ungrouped ? 'ungrouped-folder' : ''}`; group.dataset.dropFolder = folder?.id || ''; const color = safeFolderColor(folder?.color); const title = ungrouped ? 'Ungrouped sets' : folder.name; const collapsed = Boolean(folder?.collapsed); group.innerHTML = `<div class="full-folder-head" style="--folder-colour:${color}"><button class="folder-toggle" data-toggle-folder="${escapeHtml(folder?.id || '')}" type="button" ${ungrouped ? 'disabled' : ''}><span>●</span>${escapeHtml(title)} <i>${collapsed ? '›' : '⌄'}</i></button>${folderControls(folder, manual)}</div>`; if (!collapsed) { const grid = document.createElement('div'); grid.className = 'deck-grid'; sets.forEach(set => { const card = document.createElement('article'); const accent = resolvedDeckSubject(set); card.className = `deck-grid-card ${set.id === state.activeSetId ? 'active' : ''} ${accent.colour ? 'has-subject' : ''}`; if (accent.colour) card.style.setProperty('--subject-accent', accent.colour); card.draggable = true; card.dataset.dragSet = set.id; card.innerHTML = `<button ${deckAccentStyle(set)} data-set="${escapeHtml(set.id)}" type="button"><span>${escapeHtml(set.name)}</span>${deckSubjectDots(set)}<small>${set.cards.length} ${set.cards.length === 1 ? 'card' : 'cards'}</small></button>${setActions(set, manual)}`; grid.append(card); }); group.append(grid); } elements.fullLibraryGrid.append(group); });
...
function renderTestQuestion() { const test = state.activeTest; const question = activeTestQuestion(); if (!test || !question) return finishTest(); const promptName = test.config.promptSide === 'front' ? question.frontLabel : question.backLabel; const answerName = test.config.promptSide === 'front' ? question.backLabel : question.frontLabel; if (question.answerType === 'choice' && !question.choices) question.choices = RecallTest.choicesFor(question, testChoicePool(question), test.config.promptSide); const feedback = test.feedback; const answered = Boolean(feedback); const input = question.answerType === 'typed' ? `<form class="test-answer-form" id="testTypedForm"><label>Type your answer<input id="testTypedInput" autocomplete="off" autofocus ${answered ? 'disabled' : ''}/></label><button class="primary-button" type="submit" ${answered ? 'disabled' : ''}>Submit <kbd>Enter</kbd></button></form>` : `<div class="test-choices">${question.choices.map((choice, index) => `<button class="test-choice ${answered ? (RecallTest.answersMatch(choice, testAnswerValue(question)) ? 'correct' : feedback.userAnswer === choice ? 'incorrect' : '') : ''}" data-test-choice="${index}" type="button" ${answered ? 'disabled' : ''}><kbd>${index + 1}</kbd><span>${escapeHtml(choice)}</span></button>`).join('')}</div>`; const progress = Math.round((test.currentIndex / test.questions.length) * 100); testMode.innerHTML = `<div class="test-shell test-running"><header class="test-head"><div><p class="eyebrow">${escapeHtml(question.deckName)}</p><h1>Question ${test.currentIndex + 1} <em>of ${test.questions.length}</em></h1><div class="test-progress"><span style="width:${progress}%"></span></div></div><div class="test-running-actions">${testTimerMarkup(test)}<button class="text-button" data-test-action="exit" type="button">Exit test <kbd>Esc</kbd></button></div></header><main class="test-question"><div class="test-question-meta"><span>${escapeHtml(promptName)} · PROMPT</span><span class="card-state ${CARD_STATES.includes(question.state) ? question.state.toLowerCase() : 'new'}">${escapeHtml(question.state)}</span></div><article class="test-card"><p>${escapeHtml(testPromptValue(question))}</p></article><p class="test-answer-label">Answer in <b>${escapeHtml(answerName)}</b></p>${input}${answered ? `<section class="test-feedback ${feedback.result}"><strong>${feedback.result === 'correct' ? 'Correct' : 'Not quite'}</strong><span>Expected answer: <b>${escapeHtml(testAnswerValue(question))}</b></span></section>` : ''}<div class="test-question-footer"><button class="text-button" data-test-action="skip" type="button" ${answered ? 'disabled' : ''}>Skip question</button>${answered ? `<button class="primary-button" data-test-action="next" type="button">${test.currentIndex === test.questions.length - 1 ? 'See results' : 'Next question'} <span>→</span></button>` : ''}</div></main></div>`; clearInterval(testTimer); testTimer = setInterval(updateTestTimer, 250); $('#testTypedForm')?.addEventListener('submit', event => { event.preventDefault(); submitTestAnswer($('#testTypedInput').value); }); updateTestTimer(); }
function finishTest(reason = 'complete') { const test = state.activeTest; if (!test || test.finishedAt) return; clearInterval(testTimer); test.endedAt = new Date().toISOString(); test.finishReason = reason; test.summary = RecallTest.scoreTest(test); test.finishedAt = test.endedAt; state.testHistory.push({ ...test }); state.openTestResultId = test.id; state.activeTest = null; save(); renderTestResults(test); }
function renderTestResults(test) { state.openTestResultId = test.id; const summary = RecallTest.scoreTest(test); const items = testReviewRows(test); const misses = RecallTest.missedQuestions({ ...test, answers: RecallTest.reviewItems(test) }); testMode.innerHTML = `<div class="test-shell test-results"><header class="test-head"><div><p class="eyebrow">${test.finishReason === 'time-expired' ? 'Time expired' : 'Test complete'}</p><h1>${summary.percentage}% <em>accuracy</em></h1><p>${summary.correct} correct · ${summary.incorrect} incorrect · ${summary.skipped} skipped · ${summary.unanswered} unanswered</p></div><div class="test-head-actions"><button class="text-button" data-test-action="history" type="button">Test history</button><button class="drawer-close" data-test-action="close" type="button" aria-label="Close results">×</button></div></header><section class="test-score-grid"><div><b>${testFormatDuration(summary.elapsedMs)}</b><span>time taken</span></div><div><b>${testFormatDuration(summary.averageMs)}</b><span>average per question</span></div><div><b>${summary.correct}/${summary.total}</b><span>final score</span></div></section><div class="test-results-grid"><section class="test-panel"><h2>Accuracy by deck</h2><ul class="test-breakdown">${testAccuracyBreakdown(RecallTest.reviewItems(test), 'deck')}</ul><h2>Accuracy by tag</h2><ul class="test-breakdown">${testAccuracyBreakdown(RecallTest.reviewItems(test), 'tag')}</ul></section><section class="test-panel test-full-review"><div class="test-review-head"><h2>Review every question</h2><input id="testReviewSearch" type="search" value="${escapeHtml(state.testReviewSearch)}" placeholder="Search question or answer" /></div><div class="test-review-filters">${['all', 'incorrect', 'skipped', 'unanswered', 'correct'].map(filter => `<button data-test-review-filter="${filter}" type="button" class="${state.testReviewFilter === filter ? 'active' : ''}">${filter === 'all' ? 'All' : filter[0].toUpperCase() + filter.slice(1)}</button>`).join('')}</div><div class="test-missed-list test-review-list">${items.length ? items.map(item => `<article class="test-review-item ${escapeHtml(item.result)}"><span>${testResultStatus(item)} · ${escapeHtml(item.deckName)} · ${escapeHtml(item.answerType === 'choice' ? 'Multiple choice' : 'Typed')} · ${item.timeMs === null ? '—' : testFormatDuration(item.timeMs)}</span><strong>${escapeHtml(item.prompt)}</strong><p>Your answer: ${escapeHtml(item.userAnswer)}<br/>Expected answer: <b>${escapeHtml(item.correctAnswer)}</b>${item.tags?.length ? `<br/><i>${escapeHtml(item.tags.join(', '))}</i>` : ''}</p></article>`).join('') : '<p class="test-muted">No questions match this filter.</p>'}</div></section></div><footer class="test-results-actions"><button class="text-button" data-test-action="study-missed" type="button" ${misses.length ? '' : 'disabled'}>Study missed cards</button><button class="text-button" data-test-action="retry-all" type="button">Retry all</button><button class="primary-button" data-test-action="retry-missed" type="button" ${misses.length ? '' : 'disabled'}>Retry missed <span>→</span></button><button class="text-button" data-test-action="close" type="button">Return to library</button></footer></div>`; }
function retryMissedTest() { const source = testResultById(state.openTestResultId); if (source) startRetryTest(source, 'missed'); }
function studyMissedTest() { const source = testResultById(state.openTestResultId); const retry = source ? RecallTest.retryQuestions(source, 'missed') : []; const cards = retry.map(question => state.sets.find(set => set.id === question.deckId)?.cards.find(card => card.id === question.id)).filter(Boolean); if (!cards.length) return; state.testStudyContext = new Map(cards.map(card => [card.id, state.sets.find(set => set.cards.includes(card))])); state.activeSetId = studySetForCard(cards[0]).id; state.queue = shuffledCopy(cards); state.currentIndex = 0; state.correct = 0; state.retry = 0; state.history = []; state.flipped = false; state.typedChecked = false; startNewSession(); testMode.hidden = true; render(); document.querySelector('#study').scrollIntoView({ behavior: 'smooth' }); showToast(`Normal study started with ${cards.length} missed card${cards.length === 1 ? '' : 's'} across ${new Set(retry.map(question => question.deckName)).size} deck${new Set(retry.map(question => question.deckName)).size === 1 ? '' : 's'}.`); }
function closeTestMode() { clearInterval(testTimer); state.activeTest = null; testMode.hidden = true; }
