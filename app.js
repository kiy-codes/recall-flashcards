...
// Folder colours are written into style and value attributes, and share files can carry any text.
function safeColour(value, fallback = FOLDER_COLORS[0]) { return /^#[0-9a-f]{6}$/i.test(String(value || '')) ? value : fallback; }
...
function renderDeck() {
  ...
  // The placeholder makes every deck a real change; a pre-selected only option could never be chosen.
  elements.bulkMoveDeck.innerHTML = '<option value="">Choose a deck…</option>' + state.sets.filter(set => set.id !== deck?.id).map(set => `<option value="${set.id}">${escapeHtml(set.name)}</option>`).join('');
...
function renderInsights() {
  ...
  const dots = points.map(point => `<circle cx="${point.x}" cy="${point.y}" r="4.5" fill="${point.inProgress ? '#71d6c8' : '#2447c2'}" stroke="#faf8f3" stroke-width="3"><title>${sessionDate(point)} · ${escapeHtml(point.deckName)}:${Math.round(point.rate * 100)}% (${point.correct}/${point.attempts})</title></circle>`).join('');
...
queueMicrotask(() => {
testMode.addEventListener('click', event => { const action = event.target.closest('[data-test-action]')?.dataset.testAction; if (action === 'history') renderTestHistory(); if (action === 'new-test') { state.pendingTestCardIds = null; openTestSetup(); } if (action === 'open-saved') { const test = testResultById(event.target.closest('[data-test-id]').dataset.testId); if (test) renderTestResults(test); } if (action === 'delete-test') deleteTestAttempt(event.target.closest('[data-test-id]').dataset.testId); if (action === 'retry-all') retryAllTest(); });
...
function review(result) {
  ...
  const reviewEvent = RecallScheduler.createReviewEvent(answeredCard, result, reviewedAt, responseTimeMs, studySetForCard(answeredCard)?.id);
...
// Share files come from other people, so every field is normalised before use.
function shareObjects(list) { return Array.isArray(list) ? list.filter(item => item && typeof item === 'object') : []; }
function openSharePreview(payload) {
  const sets = shareObjects(payload?.sets).map(set => ({ ...set, name: String(set.name || '').trim() || 'Shared deck', frontLabel: String(set.frontLabel || ''), backLabel: String(set.backLabel || ''), cards: shareObjects(set.cards) }));
  const folders = shareObjects(payload?.folders).map(folder => ({ ...folder, name: String(folder.name || '').trim() || 'Shared folder' }));
  const rows = sets.flatMap((set, setIndex) => set.cards.map((card, index) => ({ id: makeId(), cells: [String(card.front || ''), String(card.back || ''), cleanTags(set.tags || card.tags).join(', '), String(card.notes || ''), String(card.hint || '')], include: true, sourceRow: `${set.name} · ${index + 1}`, sourceSetIndex: setIndex, original: card })));
  state.importDraft = { kind: 'share', headers: ['First side', 'Second side', 'Tags', 'Notes', 'Hint'], rows, folders, sets, defaults: { front: 0, back: 1, tags: 2, notes: 3, hint: 4 } }; renderImportPreview(); elements.importPreview.hidden = false;
}
// A shared card keeps the sender's learning data; the review screen decides whether
// it is imported and can change its text or gender.
function sharedCardFromRow(row, draft) {
  const original = row.original || {}; const mapped = importMappedCard(row, draft); const corrected = row.genderCorrection && row.genderCorrection !== 'auto';
  return { ...original, front: corrected ? mapped.front : importRead(row, 'front', draft).trim(), back: mapped.back, notes: mapped.notes, hint: mapped.hint, ...(corrected ? { wordInfo: { ...(original.wordInfo || {}), gender: mapped.wordInfo.gender, originalMarker: mapped.wordInfo.originalMarker } } : {}) };
}
...
function renderImportFlow() {
  ...
  if (draft.kind === 'share') elements.importPreview.querySelector('.import-destination-grid')?.replaceWith(Object.assign(document.createElement('p'), { className: 'import-existing-meta', textContent: 'Shared cards go into your deck with the same name, or a new deck with the sender’s details and folder.' }));
...
function confirmImportFlow() {
  ...
  if (draft.kind === 'share') {
    ...
    (draft.sets || []).forEach((sourceSet, setIndex) => {
      const cards = draft.rows.filter(row => row.include && row.sourceSetIndex === setIndex).map(row => sharedCardFromRow(row, draft)).filter(card => card.front && card.back);
      if (!cards.length) return;
      let destination = state.sets.find(set => set.name === sourceSet.name);
      if (!destination) { destination = createSetForImport(sourceSet.name); applyImportMetadata(destination, { subject: sourceSet.subject, domain: sourceSet.domain, language: sourceSet.language, tags: sourceSet.tags }); destination.frontLabel = sourceSet.frontLabel || 'First side'; destination.backLabel = sourceSet.backLabel || 'Second side'; const sourceFolder = draft.folders?.find(folder => folder.id === sourceSet.folderId); if (sourceFolder) { let folder = foldersByName.get(sourceFolder.name); if (!folder) { folder = { id: makeId(), name: sourceFolder.name, color: safeColour(sourceFolder.color, FOLDER_COLORS[state.folders.length % FOLDER_COLORS.length]), order: state.folders.length }; state.folders.push(folder); foldersByName.set(folder.name, folder); } destination.folderId = folder.id; } }
      imported += addCards(cards, duplicateMode, destination);
    });
...
elements.manageTags.addEventListener('click', openTagManager); elements.tagDialogClose.addEventListener('click', closeTagManager); 
elements.newTag.addEventListener('click', () => { const tag = prompt('Name the new deck tag:'); if (!tag) return; activeSet().tags = cleanTags([...(activeSet().tags || []), tag]); save(); render(); renderTagManager(); });
elements.tagDialogList.addEventListener('click', event => { const rename = event.target.dataset.renameTag; const remove = event.target.dataset.deleteTag; if (!rename && !remove) return; if (rename) renameTag(rename, prompt('Rename deck tag:', rename)); if (remove && confirm(`Delete the “${remove}” tag from all decks?`)) deleteTag(remove); });
...
function testFolderChoices() {
  return state.folders.map(folder => `<label class="test-check"><input data-test-folder type="checkbox" value="${folder.id}"/><span><i style="background:${safeColour(folder.color)}"></i>${escapeHtml(folder.name)}</span><small>${state.sets.filter(set => set.folderId === folder.id).length} decks</small></label>`).join('') || '<p class="test-muted">No folders yet.</p>';
}
...
function renderLibrary() {
  ...
  libraryGroups(filter, mode).forEach(({ folder, sets, ungrouped }) => { const group = document.createElement('section'); group.className = `library-folder ${ungrouped ? 'ungrouped-folder' : ''}`; group.dataset.dropFolder = folder?.id || ''; const color = safeColour(folder?.color, '#a0a6b4'); const title = ungrouped ? 'Ungrouped sets' : folder.name; const collapsed = Boolean(folder?.collapsed); group.innerHTML = `<div class="library-folder-head" style="--folder-colour:${color}"><button class="folder-toggle" data-toggle-folder="${folder?.id || ''}" type="button" ${ungrouped ? 'disabled' : ''}><span class="folder-symbol">●</span><span>${escapeHtml(title)}</span><i>${collapsed ? '›' : '⌄'}</i></button>${folderControls(folder, manual)}</div>`; if (!collapsed) sets.forEach(set => { const row = document.createElement('div'); row.className = 'library-set-row'; row.draggable = true; row.dataset.dragSet = set.id; row.innerHTML = `<button class="set-entry ${set.id === state.activeSetId ? 'active' : ''} ${resolvedDeckSubject(set).colour ? 'has-subject' : ''}" ${deckAccentStyle(set)} data-set="${set.id}" type="button"><span>${escapeHtml(set.name)}</span>${deckSubjectDots(set)}<small>${set.cards.length} ${set.cards.length === 1 ? 'card' : 'cards'}</small></button>${setActions(set, manual)}`; group.append(row); }); elements.libraryTree.append(group); });
...
deckSubjectButton.addEventListener('click', renderDeckMetadataDialog); subjectColoursButton.addEventListener('click', renderSubjectColours);
...
// Test Mode setup, questions and results.
...
function studyMissedTest() { const source = testResultById(state.openTestResultId); const retry = source ? RecallTest.retryQuestions(source, 'missed') : []; const cards = retry.map(question => state.sets.find(set => set.id === question.deckId)?.cards.find(card => card.id === question.id)).filter(Boolean); if (!cards.length) return; state.testStudyContext = new Map(cards.map(card => [card.id, state.sets.find(set => set.cards.includes(card))])); state.activeSetId = studySetForCard(cards[0]).id; finishCurrentSession(); state.queue = shuffledCopy(cards); state.currentIndex = 0; state.correct = 0; state.retry = 0; state.history = []; state.flipped = false; state.typedChecked = false; state.sessionRepeatedIds = new Set(); state.hintRevealed = false; state.cardPresentedAt = Date.now(); startNewSession(); testMode.hidden = true; render(); document.querySelector('#study').scrollIntoView({ behavior: 'smooth' }); showToast(`Normal study started with ${cards.length} missed card${cards.length === 1 ? '' : 's'} across ${new Set(retry.map(question => question.deckName)).size} deck${new Set(retry.map(question => question.deckName)).size === 1 ? '' : 's'}.`); }
function closeTestMode() { clearInterval(testTimer); state.activeTest = null; state.pendingTestCardIds = null; testMode.hidden = true; }
...
elements.bulkTag.addEventListener('click', () => { showToast('Tags belong to the whole deck. Edit Deck details to change them.'); });
elements.bulkRemoveTag.addEventListener('click', () => { showToast('Tags belong to the whole deck. Edit Deck details to change them.'); });
