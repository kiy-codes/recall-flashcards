...
export function importShare(library, payload, mode = 'merge', duplicateMode = 'skip') { const next = migrateLibrary(library); if (!payload || payload.format !== 'recall-mobile-share-v1') return { library: next, imported: 0 }; const folders = new Map(next.folders.map(folder => [folder.name, folder])); let imported = 0; payload.sets.forEach(source => { let target = next.sets.find(set => set.name === source.name); if (mode === 'skip' && target) return; if (mode === 'copy' || !target) { target = { ...source, id: id(), name: mode === 'copy' && next.sets.some(set => set.name === source.name) ? `${source.name} copy` : source.name, cards: [], order: next.sets.length }; const sourceFolder = payload.folders.find(folder => folder.id === source.folderId); if (sourceFolder) { let folder = folders.get(sourceFolder.name); if (!folder) { folder = { ...sourceFolder, id: id(), order: next.folders.length }; next.folders.push(folder); folders.set(folder.name, folder); } target.folderId = folder.id; } next.sets.push(target); }
    source.cards.map(normaliseCard).forEach(card => { const existing = findDuplicate(target.cards, card); if (existing && duplicateMode === 'skip') return; if (existing && duplicateMode === 'replace') Object.assign(existing, { ...card, id: existing.id }); else target.cards.push(card); imported += 1; });
