...
export function migrateLibrary(raw) {
  ...
  const sets = Array.isArray(base.sets) && base.sets.length ? base.sets.map((set, index) => ({ id: set.id || id(), name: set.name || `Study set ${index + 1}`, folderId: set.folderId || null, frontLabel: set.frontLabel || 'First side', backLabel: set.backLabel || 'Second side', order: Number.isFinite(set.order) ? set.order : index, cards: Array.isArray(set.cards) ? set.cards.map(normaliseCard) : [] })) : blankLibrary().sets;
  return { version: 3, sets, folders: Array.isArray(base.folders) ? base.folders.map((folder, index) => ({ id: folder.id || id(), name: folder.name || `Folder ${index + 1}`, color: folder.color || '#2447c2', order: Number.isFinite(folder.order) ? folder.order : index, collapsed: Boolean(folder.collapsed) })) : [], activeSetId: sets.some(set => set.id === base.activeSetId) ? base.activeSetId : sets[0].id, sessions: Array.isArray(base.sessions || base.sessionHistory) ? (base.sessions || base.sessionHistory).map(session => ({ ...session, id: session.id || id(), learned: session.learned || 0 })) : [], activity: base.activity && typeof base.activity === 'object' ? base.activity : {} };
}
...
const shareText = (value, max) => String(value || '').trim().slice(0, max);
const shareColor = value => /^#[0-9a-f]{6}$/i.test(value || '') ? value : '#2447c2';
export const sharePayload = (library, setIds) => {
  const sets = library.sets.filter(set => setIds.includes(set.id));
  const folders = library.folders.filter(folder => sets.some(set => set.folderId === folder.id));
  return { format: 'recall-mobile-share-v1', exportedAt: new Date().toISOString(), folders: folders.map(folder => ({ id: shareText(folder.id, 128), name: shareText(folder.name, 70), color: shareColor(folder.color) })), sets: sets.map(set => ({ id: shareText(set.id, 128), name: shareText(set.name, 70), folderId: set.folderId ? shareText(set.folderId, 128) : null, frontLabel: shareText(set.frontLabel, 40), backLabel: shareText(set.backLabel, 40), cards: set.cards.slice(0, 2000).map(card => ({ front: shareText(card.front, 700), back: shareText(card.back, 700), tags: cleanTags(card.tags).slice(0, 30).map(tag => shareText(tag, 100)).filter(Boolean), })) })) };
};
export function importShare(library, payload, mode = 'merge', duplicateMode = 'skip') {
  const next = migrateLibrary(library);
  if (!payload || payload.format !== 'recall-mobile-share-v1' || !Array.isArray(payload.sets) || !Array.isArray(payload.folders) || payload.sets.length > 100 || payload.folders.length > 100 || payload.sets.some(set => !set || typeof set.name !== 'string' || !Array.isArray(set.cards) || set.cards.length > 2000)) return { library: next, imported: 0 };
  const folders = new Map(next.folders.map(folder => [folder.name, folder])); let imported = 0;
  payload.sets.forEach(source => {
    const name = shareText(source.name, 70); if (!name) return;
    let target = next.sets.find(set => set.name === name); if (mode === 'skip' && target) return;
    if (mode === 'copy' || !target) {
      target = { id: id(), name: mode === 'copy' && target ? `${name} copy` : name, folderId: null, frontLabel: shareText(source.frontLabel, 40) || 'First side', backLabel: shareText(source.backLabel, 40) || 'Second side', cards: [], order: next.sets.length };
      const sourceFolder = payload.folders.find(folder => folder?.id === source.folderId && typeof folder.name === 'string');
      if (sourceFolder) { const folderName = shareText(sourceFolder.name, 70); let folder = folders.get(folderName); if (!folder && folderName) { folder = { id: id(), name: folderName, color: shareColor(sourceFolder.color), order: next.folders.length }; next.folders.push(folder); folders.set(folderName, folder); } if (folder) target.folderId = folder.id; }
      next.sets.push(target);
    }
    source.cards.forEach(raw => { if (!raw || typeof raw !== 'object') return; const card = normaliseCard({ front: shareText(raw.front, 700), back: shareText(raw.back, 700), tags: cleanTags(raw.tags).slice(0, 30).map(tag => shareText(tag, 100)).filter(Boolean) }); if (!card.front || !card.back) return; const existing = findDuplicate(target.cards, card); if (existing && duplicateMode === 'skip') return; if (existing && duplicateMode === 'replace') Object.assign(existing, { ...card, id: existing.id }); else target.cards.push(card); imported += 1; });
