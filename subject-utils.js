(function (root, factory) {
  ...
  const subjectsForDeck = deck => Array.from(new Set([...(deck?.tags || []), ...(deck?.cards || []).flatMap(card => card.tags || [])].map(parseSubjectTag).filter(Boolean).map(item => item.subject)));
  const primarySubject = (deck, subjects = subjectsForCards(deck?.cards)) => { const chosen = String(deck?.primarySubject || '').trim(); if (chosen && subjects.includes(chosen)) return { subject: chosen, fallback: false }; return { subject: subjects[0] || null, fallback: Boolean(chosen && !subjects.includes(chosen)) }; };
